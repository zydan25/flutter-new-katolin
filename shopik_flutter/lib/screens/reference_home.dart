              ...
              itemBuilder: (_, index) {
                ...
                return InkWell(
                  ...
                  child: Container(
                    ...
                    decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(17), border: Border.all(color: AppColors.border)),
                    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      Container(width: 36, height: 36, decoration: BoxDecoration(color: shortcut.color, borderRadius: BorderRadius.circular(11)), child: Icon(shortcut.icon, color: Colors.white, size: 20)),
                      const Spacer(),
                      Text(shortcut.title, maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(fontSize: 11.5, fontWeight: FontWeight.w900)),
                      const SizedBox(height: 2),
...
class _PaymentScreenState extends State<PaymentScreen> {
  ...
  void initState() {
    ...
    if (app.catalogServices.isEmpty) app.refreshCatalog();
  }
  ...
  Widget build(BuildContext context) {
    ...
    final labels = const <String, String>{
      'yemen_mobile': 'يمن موبايل', 'you': 'YOU', 'sabafon': 'سبأفون', 'y': 'واي', 'yemen4g': 'يمن 4G', 'yemen_net': 'يمن نت', 'aden_net': 'عدن نت',
    };
    ...
    return ScreenFrame(
      title: 'شبكة السداد',
      color: color,
      actions: [IconButton(onPressed: app.refreshCatalog, icon: const Icon(Icons.sync_rounded))],
      child: RefreshIndicator(
        color: color,
