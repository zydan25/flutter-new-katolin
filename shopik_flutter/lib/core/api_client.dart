...
class ApiClient {
  ...
  Uri _uri(String path, [Map<String, dynamic>? query]) {
    ...
    return Uri.parse('$baseUrl$clean').replace(queryParameters: query?.map((k, v) => MapEntry(k, v.toString())));
  }
  ...
  Future<dynamic> get(String path, {Map<String, dynamic>? query}) async {
    final response = await _client.get(_uri(path, query), headers: await _headers()).timeout(_timeout);
    _check(response);
  ...
  Future<dynamic> post(String path, dynamic body, {String? idempotencyKey}) async {
    ...
    final response = await _client.post(_uri(path), headers: headers, body: jsonEncode(body)).timeout(_timeout);
    _check(response);
    return _decode(response);
  }

  Future<dynamic> patch(String path, dynamic body) async {
    final response = await _client.patch(_uri(path), headers: await _headers(json: true), body: jsonEncode(body)).timeout(_timeout);
    _check(response);
    return _decode(response);
  }

  Future<dynamic> delete(String path) async {
    final response = await _client.delete(_uri(path), headers: await _headers()).timeout(_timeout);
    _check(response);
    return _decode(response);
  }
  ...
  Future<Map<String, dynamic>> register({
    ...
    final data = await post('/auth/register/', {
      ...
      'third_name': thirdName.trim(),
      'last_name': lastName.trim(),
      'governorate': governorate.trim(),
    });
    if (data is! Map || data['token'] == null) throw ApiException(200, 'تم التسجيل لكن الخادم لم يُرجع رمز الدخول', data);
    await saveToken(data['token'].toString());
    return Map<String, dynamic>.from(data);
  }
  ...
  // Compatibility wallet endpoint remains available, but is not used as the financial source of truth.
  ...
  Future<Map<String, dynamic>> serviceRequest({required int serviceId, required Map<String, dynamic> payload, String? itemType, int? itemId, String? idempotencyKey}) async {
    final key = idempotencyKey ?? '${DateTime.now().microsecondsSinceEpoch}-${Random().nextInt(1 << 30)}';
    return Map<String, dynamic>.from(await post('/v2/services/requests/', {
  ...
  Future<Map<String, dynamic>> wifiPurchase({required int networkId, required int denominationId, required String phone, required double price, String? idempotencyKey}) async {
    final key = idempotencyKey ?? 'wifi-${DateTime.now().microsecondsSinceEpoch}-${Random().nextInt(1 << 30)}';
    return Map<String, dynamic>.from(await post('/v2/services/wifi/purchase/', {'network_id': networkId, 'denomination_id': denominationId, 'phone': phone, 'amount': price}, idempotencyKey: key));
  }
  Future<List<Map<String, dynamic>>> wifiCards() async {
    final data = await get('/v2/services/wifi/my-cards/');
    if (data is Map && data['cards'] is List) return List<Map<String, dynamic>>.from((data['cards'] as List).map((e) => Map<String, dynamic>.from(e)));
    return _results(data);
  }

  // Store/catalog compatibility routes
  ...
  Future<Map<String, dynamic>> createOrder({required List<Map<String, dynamic>> items, required Map<String, dynamic> shippingAddress, String currency = 'YER', String paymentMethod = 'wallet', String couponCode = ''}) async {
    final key = 'order-${DateTime.now().microsecondsSinceEpoch}-${Random().nextInt(1 << 30)}';
    return Map<String, dynamic>.from(await post('/orders/', {'items': items, 'shipping_address': shippingAddress, 'currency': currency, 'payment_method': paymentMethod, if (couponCode.isNotEmpty) 'coupon_code': couponCode}, idempotencyKey: key));
  }
  ...
  // Finance: authoritative accounting transfer/gift contracts
  Future<Map<String, dynamic>> transfer({required String recipient, required double amount, String currency = 'YER', String note = '', String? idempotencyKey}) async {
    final key = idempotencyKey ?? 'transfer-${DateTime.now().microsecondsSinceEpoch}-${Random().nextInt(1 << 30)}';
    return Map<String, dynamic>.from(await post('/v2/accounting/transfers/', {'recipient': recipient.trim(), 'amount': amount, 'currency': currency, if (note.trim().isNotEmpty) 'note': note.trim()}, idempotencyKey: key));
  }
  Future<Map<String, dynamic>> gift({required String recipient, required double amount, String currency = 'YER', String message = '', String? idempotencyKey}) async {
    final key = idempotencyKey ?? 'gift-${DateTime.now().microsecondsSinceEpoch}-${Random().nextInt(1 << 30)}';
    return Map<String, dynamic>.from(await post('/v2/accounting/gifts/', {'recipient': recipient.trim(), 'amount': amount, 'currency': currency, if (message.trim().isNotEmpty) 'message': message.trim()}, idempotencyKey: key));
  }

  // Compatibility gift endpoints retained for lookup/history only.
  ...
  Future<Map<String, dynamic>> createGift({required String receiverPhone, required double amount, String message = ''}) => gift(recipient: receiverPhone, amount: amount, message: message);
