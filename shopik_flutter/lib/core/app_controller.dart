...
class AppController extends ChangeNotifier {
  ...
  Future<void> restore() async {
    ...
    try { await refreshWalletAndReports(); await refreshOptional(); _startPolling(); }
  ...
  Future<bool> login(String identifier, String password) async {
    ...
    try {
      ...
      user = UserProfile.fromJson(Map<String,dynamic>.from(data['user'] ?? {}));
      final prefs = await SharedPreferences.getInstance();
  ...
  Future<void> logout({bool localOnly=false}) async {
    ...
    user = null; walletBalance = 0; operations = []; orders = []; notifications = []; serviceCatalogRoots = [];
  ...
  Future<void> refreshWalletAndReports() async {
    ...
    if (rows.isNotEmpty) walletBalance = num.tryParse('${rows.first['balance'] ?? 0}') ?? walletBalance;
  ...
  Future<void> refreshOptional() async {
    ...
    await safe(() async {
      final d = await api.serviceCatalog();
  ...
  Future<void> refreshAll() async {
    loading = true; error = null; notifyListeners();
    try { await refreshWalletAndReports(); await refreshOptional(); }
  ...
  List<Map<String,dynamic>> get catalogServices {
    ...
    void walk(dynamic node) {
      ...
      if (services is List) for (final e in services) { if (e is Map) out.add(Map<String,dynamic>.from(e)); }
      final a = node['categories']; if (a is List) for (final e in a) walk(e);
      final b = node['children']; if (b is List) for (final e in b) walk(e);
    }
