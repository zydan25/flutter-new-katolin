    ...
    Card(Modifier.fillMaxWidth(), RoundedCornerShape(22.dp), colors = CardDefaults.cardColors(White), elevation = CardDefaults.cardElevation(4.dp)) {
        Column(Modifier.padding(14.dp), verticalArrangement = Arrangement.spacedBy(10.dp)) {
            ...
            Row(Modifier.fillMaxWidth(), verticalAlignment = Alignment.CenterVertically) {
                ...
                Surface(
                    modifier = Modifier.size(46.dp).clickable { onPickContact() },
                    shape = RoundedCornerShape(12.dp),
                    color = provider.color.copy(alpha = 0.12f)
                ) {
                    Box(contentAlignment = Alignment.Center) {
                        Icon(Icons.Default.Phone, contentDescription = "جهات الاتصال", tint = provider.color, modifier = Modifier.size(22.dp))
                    }
                }
            }
...
fun PaymentNetworkScreen(
    ...
    LaunchedEffect(baseUrl, session.token) {
        restoreCatalog()
        // Only fetch from network if catalog is empty in local storage!
        if (catalog.isEmpty() && session.token != null) syncCatalog()
    }
                ...
                context.contentResolver.query(contactUri, projection, null, null, null)?.use { cursor ->
                    if (cursor.moveToFirst()) {
                        ...
                        if (numberIndex != -1) {
                            ...
                            val cleaned = raw.replace(Regex("[^0-9]"), "").removePrefix("967").removePrefix("00967")
                            phone = cleaned
                        }
                    }
                }
    ...
    var pendingPurchaseItem by remember { mutableStateOf<ServiceItemDto?>(null) }

    fun checkDuplicateToday(targetPhone: String): Boolean {
        val today = java.text.SimpleDateFormat("yyyy-MM-dd", java.util.Locale.US).format(java.util.Date())
        val key = "recharge_${targetPhone}_$today"
        return SessionStore.loadLocalString(key) != null
    }

    fun recordPaymentToday(targetPhone: String, desc: String) {
        val today = java.text.SimpleDateFormat("yyyy-MM-dd", java.util.Locale.US).format(java.util.Date())
        val key = "recharge_${targetPhone}_$today"
        SessionStore.saveLocalString(key, desc)
    }

    fun runLoanOnlyQuery() {
        val token = session.token ?: return
        val service = loanQuery ?: run { error = "خدمة فحص السلفة غير مهيأة."; return }
        scope.launch {
            loading = true; error = null; operationResult = null
            try {
                loanResult = submitAndPoll(baseUrl, token, service, phone, "", netType, purchase = false)
            } catch (e: Exception) { error = e.localizedMessage ?: "تعذر فحص السلفة." }
            finally { loading = false }
        }
    }
    ...
    fun runAction(next: PaymentAction) {
        ...
        showConfirm = false
        when (next) {
            PaymentAction.BALANCE, PaymentAction.PACKAGES -> { /* Independent explicit inquiries */ }
    ...
    fun promptPurchase(item: ServiceItemDto? = null, service: ServiceDto? = null) {
        selectedItem = item
        selectedPurchaseService = service ?: when (action) {
            PaymentAction.BALANCE -> balancePurchase
            PaymentAction.PACKAGES -> packagePurchase
            else -> selectedPurchaseService
        }
        if (checkDuplicateToday(phone)) {
        ...
        scope.launch {
            ...
            try {
                ...
                if (latest.status == "success") {
                    recordPaymentToday(phone, item?.name ?: service.name)
                    onSyncBalance()
    ...
    Scaffold(
        ...
        topBar = {
            TopAppBar(
                ...
                actions = {
                    Surface(
                        modifier = Modifier.padding(end = 4.dp).clickable { onOpenFlutter() },
                        shape = RoundedCornerShape(8.dp),
                        color = White.copy(alpha = 0.25f)
                    ) {
                        Text("واجهة فلاتر", color = White, fontWeight = FontWeight.Bold, fontSize = 11.sp, modifier = Modifier.padding(horizontal = 8.dp, vertical = 4.dp))
                    }
                    IconButton(onClick = onBackClick) { Text("رجوع", color = White, fontWeight = FontWeight.Bold, fontSize = 11.sp) }
                },
        ...
        LazyColumn(Modifier.fillMaxSize().padding(padding).background(ScreenBg), contentPadding = PaddingValues(horizontal = 12.dp, vertical = 10.dp), verticalArrangement = Arrangement.spacedBy(9.dp)) {
            ...
            item {
                Surface(
                    modifier = Modifier.fillMaxWidth().clickable { onOpenFlutter() },
                    shape = RoundedCornerShape(12.dp),
                    ...
                    Row(
                        ...
                        Text("فتح واجهة فلاتر المتكاملة (Flutter Telecom UI)", color = White, fontWeight = FontWeight.Bold, fontSize = 12.sp)
                    }
                }
            }

            // Independent Inquiries Bar
            item {
                Surface(
                    modifier = Modifier.fillMaxWidth(),
                    ...
                    Column(Modifier.padding(10.dp), verticalArrangement = Arrangement.spacedBy(8.dp)) {
                        Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween, verticalAlignment = Alignment.CenterVertically) {
                            Text("استعلامات وفحص الرقم", color = provider.color, fontWeight = FontWeight.Black, fontSize = 13.sp)
                            if (loading) CircularProgressIndicator(modifier = Modifier.size(16.dp), color = provider.color, strokeWidth = 2.dp)
                        }
                        if (detectedKind == ProviderKind.YEMEN_MOBILE) {
                            Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(6.dp)) {
                                Button(
                                    onClick = ::runBalanceQuery,
                                    modifier = Modifier.weight(1f).height(40.dp),
                                    colors = ButtonDefaults.buttonColors(containerColor = provider.color.copy(alpha = 0.12f)),
                                    shape = RoundedCornerShape(10.dp)
                                ) {
                                    Text("فحص الرصيد", color = provider.color, fontWeight = FontWeight.Bold, fontSize = 11.sp)
                                }
                                Button(
                                    onClick = ::runLoanOnlyQuery,
                                    modifier = Modifier.weight(1f).height(40.dp),
                                    colors = ButtonDefaults.buttonColors(containerColor = provider.color.copy(alpha = 0.12f)),
                                    shape = RoundedCornerShape(10.dp)
                                ) {
                                    Text("فحص السلفة", color = provider.color, fontWeight = FontWeight.Bold, fontSize = 11.sp)
                                }
                                Button(
                                    onClick = ::runPackageQuery,
                                    modifier = Modifier.weight(1f).height(40.dp),
                                    colors = ButtonDefaults.buttonColors(containerColor = provider.color.copy(alpha = 0.12f)),
                                    shape = RoundedCornerShape(10.dp)
                                ) {
                                    Text("فحص الباقات", color = provider.color, fontWeight = FontWeight.Bold, fontSize = 11.sp)
                                }
                            }
                        } else if (detectedKind == ProviderKind.YEMEN_4G) {
                            Button(
                                onClick = ::runBalanceQuery,
                                ...
                                Text("استعلام رصيد وصلاحية يمن فورجي", color = provider.color, fontWeight = FontWeight.Bold, fontSize = 12.sp)
                            }
                        } else if (detectedKind == ProviderKind.YEMEN_NET) {
                            Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(6.dp)) {
                                Button(
                                    onClick = { netType = "adsl"; runBalanceQuery() },
                                    modifier = Modifier.weight(1f).height(40.dp),
                                    colors = ButtonDefaults.buttonColors(containerColor = provider.color.copy(alpha = 0.12f)),
                                    shape = RoundedCornerShape(10.dp)
                                ) {
                                    Text("استعلام رصيد ADSL", color = provider.color, fontWeight = FontWeight.Bold, fontSize = 11.sp)
                                }
                                Button(
                                    onClick = { netType = "line"; runBalanceQuery() },
                                    modifier = Modifier.weight(1f).height(40.dp),
                                    colors = ButtonDefaults.buttonColors(containerColor = provider.color.copy(alpha = 0.12f)),
                                    shape = RoundedCornerShape(10.dp)
                                ) {
                                    Text("استعلام الهاتف الثابت", color = provider.color, fontWeight = FontWeight.Bold, fontSize = 11.sp)
                                }
                            }
                        }
                    }
            ...
            if (action == PaymentAction.BALANCE) {
                ...
                item {
                    Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                        ...
                        Button(onClick = { promptPurchase(service = balancePurchase) }, enabled = balancePurchase != null && !loading, modifier = Modifier.weight(1f).height(50.dp), colors = ButtonDefaults.buttonColors(containerColor = Color(0xFFFFDDAE)), shape = RoundedCornerShape(12.dp)) { Text("تسديد", color = provider.color, fontWeight = FontWeight.Black, fontSize = 16.sp) }
                    }
                }
            ...
            if (action == PaymentAction.PACKAGES) {
                ...
                if (purchaseItems.isNotEmpty()) {
                    ...
                    purchaseItems.forEach { itemDto ->
                        item { PackagePurchaseCard(itemDto, provider) { promptPurchase(item = itemDto, service = packagePurchase) } }
                    }
                } else {
            ...
            if (action == PaymentAction.INSTANT) {
                ...
                if (instantItems.isNotEmpty()) {
                    ...
                    instantItems.forEach { itemDto ->
                        item { DenominationPurchaseCard(itemDto, provider) { promptPurchase(item = itemDto, service = denominationPurchase) } }
                    }
                } else {
                    item { AmountCard(amount, { amount = it }, provider) }
                    item {
                        Button(onClick = { promptPurchase(service = denominationPurchase) }, enabled = denominationPurchase != null && !loading, modifier = Modifier.fillMaxWidth().height(52.dp), colors = ButtonDefaults.buttonColors(containerColor = provider.color), shape = RoundedCornerShape(13.dp)) {
                ...
                item { AmountCard(amount, { amount = it }, provider) }
                item {
                    Button(onClick = { promptPurchase(service = detectedKind?.let { providerService(allServices, it, action, true) }) }, enabled = !loading, modifier = Modifier.fillMaxWidth().height(52.dp), colors = ButtonDefaults.buttonColors(containerColor = provider.color), shape = RoundedCornerShape(13.dp)) {
                        Text("تسديد", fontWeight = FontWeight.Black, fontSize = 16.sp)
                    }
                }
            }
        ...
        AlertDialog(
            ...
            title = {
                Row(verticalAlignment = Alignment.CenterVertically) {
                    ...
                    Text("تنبيه تكرار السداد اليوم", fontWeight = FontWeight.Bold, fontSize = 16.sp)
                }
            },
            text = {
                Text(
                    "لقد قمت مسبقاً بعملية سداد لهذا الرقم ($phone) اليوم.\n\nهل تريد تأكيد إعادة السداد لنفس الرقم؟",
                    fontSize = 13.sp,
                    lineHeight = 18.sp
                )
            },
            confirmButton = {
                Button(
                    ...
                    Text("نعم، تابع السداد", fontWeight = FontWeight.Bold)
                }
            },
            dismissButton = {
